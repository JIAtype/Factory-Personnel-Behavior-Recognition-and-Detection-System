...
class BehaviorDetectionSystem:
    ...
    def detection_loop(self):
        while self.running:
            ...
            # 将【带标注】的帧存入录制缓冲区,保存的是处理后的结果
            self.recording_buffer.append((time.time(), annotated_frame.copy()))
        ...
        for tracker_id, tracker in self.trackers.items():
            ...
            if self.helmet_class_id is not None:
                if not tracker.has_helmet:
                    if color == (0, 255, 0): color = (255, 0, 255)
    ...
    def trigger_alert(self, message, person_id, alert_type):
        ...
        # 只有在没有其他录制任务正在进行时，才启动新的录制
        if not self.is_recording_alert_active:
            self.is_recording_alert_active = True # 立即设置标志，防止并发触发
            # 将录制任务放入新线程，避免阻塞主检测循环
            threading.Thread(target=self.save_alert_recording, args=(message,)).start()
