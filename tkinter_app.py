...
class PersonTracker:
    def __init__(self, person_id, keypoints):
        ...
        self.fall_alert_sent = False  # Prevent duplicate fall alerts
        self.stationary_alert_sent = False  # Prevent duplicate stationary alerts
        self.last_alert_time = 0  # Last time alert was sent
    ...
    def update(self, keypoints):
        ...
        if len(self.keypoints_history) >= 2:
            ...
            if movement > 10:  # Threshold for movement detection
                self.last_movement_time = time.time()
                # Reset alerts when person starts moving
                if self.is_stationary:
                    self.stationary_alert_sent = False
                if self.is_fallen:
                    self.fall_alert_sent = False
                self.is_stationary = False
                self.is_fallen = False  # Reset fall detection when moving
            else:
        ...
        if fall_detected and not self.is_fallen:
            self.is_fallen = True
            self.fall_alert_sent = False  # Allow new fall alert
...
class BehaviorDetectionSystem:
    def __init__(self, video_source="test_video.mp4"):
        ...
        self.alert_cooldown = {}  # Track alert cooldowns per person
        self.enable_popup_alerts = True  # User can toggle popup alerts
        self.enable_sound_alerts = True  # User can toggle sound alerts
    ...
    def setup_gui(self):
        ...
        # Alert controls
        alert_controls_frame = ttk.Frame(alerts_frame)
        alert_controls_frame.pack(fill=tk.X, pady=(5, 0))
        
        ttk.Button(alert_controls_frame, text="Clear Alerts", command=self.clear_alerts).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(alert_controls_frame, text="Mute All", command=self.toggle_sound).pack(side=tk.LEFT)
        ...
        ttk.Label(settings_frame, text="Alert Settings:").pack(anchor=tk.W)
        
        self.popup_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(settings_frame, text="Enable Popup Alerts", variable=self.popup_var, 
                       command=self.toggle_popup_alerts).pack(anchor=tk.W)
        
        self.sound_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(settings_frame, text="Enable Sound Alerts", variable=self.sound_var,
                       command=self.toggle_sound_alerts).pack(anchor=tk.W)
    ...
    def update_video_source(self):
            ...
            self.add_alert_text(f"[SYSTEM] Video source updated to: {source_type}\n")
    ...
    def toggle_sound(self):
        ...
        self.enable_sound_alerts = not self.enable_sound_alerts
        status = "enabled" if self.enable_sound_alerts else "disabled"
        self.add_alert_text(f"[SYSTEM] Sound alerts {status}\n")
    
    def toggle_popup_alerts(self):
        """Toggle popup alerts on/off"""
        self.enable_popup_alerts = self.popup_var.get()
    
    def toggle_sound_alerts(self):
        """Toggle sound alerts on/off"""
        self.enable_sound_alerts = self.sound_var.get()
    ...
    def start_detection(self):
        ...
        if not self.cap.isOpened():
            self.add_alert_text(f"[ERROR] Failed to open video source: {self.video_source}\n")
            return
        ...
        if self.is_video_file:
            ...
            self.add_alert_text(f"[INFO] Loaded video file - Duration: {duration:.1f}s, FPS: {fps:.1f}\n")
        else:
            self.add_alert_text("[INFO] Connected to RTSP stream\n")
        ...
        while self.running:
            ...
            if not ret:
                if self.is_video_file:
                    ...
                    self.add_alert_text("[WARNING] Connection lost, trying to reconnect...\n")
                    time.sleep(5)
                    self.cap = cv2.VideoCapture(self.video_source)
                    continue
    ...
    def calculate_detection_distance(self, tracker, detection):
        ...
        # Calculate center of mass distance with safety checks
        try:
    ...
    def draw_annotations(self, frame):
        ...
        for tracker_id, tracker in self.trackers.items():
            ...
            # Draw bounding box and status with safety checks
            try:
                ...
                if len(valid_keypoints) > 0:
                    ...
                    if tracker.is_fallen:
                        ...
                        fallen_count += 1
                        self.trigger_alert(f"FALL DETECTED - Person {tracker_id}", tracker_id, "fall")
                    elif tracker.is_stationary:
                        color = (0, 165, 255)  # Orange for stationary
                        status_text = f"Person {tracker_id}: STATIONARY"
                        stationary_count += 1
                        self.trigger_alert(f"STATIONARY ALERT - Person {tracker_id}", tracker_id, "stationary")
                    
                    cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), color, 2)
                    cv2.putText(annotated_frame, status_text, (x1, y1-10), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
            except Exception as e:
                print(f"Error drawing annotations for tracker {tracker_id}: {e}")
                continue
